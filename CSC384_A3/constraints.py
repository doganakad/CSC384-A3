...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for domI in qi.curDomain():
            for domJ in qj.curDomain():
                if (domI != domJ) and (abs(domI-domJ) != abs(i-j)):
                    satisfyingAssignments.append([domI,domJ])
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        return self._lb <= len(result) <= self._ub
