...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint('diffRow', row))
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint('diffCol', scope))
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(AllDiffConstraint('diffSq', scope))
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    planes_list = PlaneCSP(planes_problem)[1] 
    result = list()
    for solution in solutions:
        ...
        for x in range(len(planes_list)):
            planes = list()
            planes.append(planes_problem.planes[x])
            for y in range(len(planes_list[x])):
                if solution[start][1]!="NULL":
                    planes.append(solution[start][1])
                start+=1
            var_list.append(planes)
        result.append(var_list)
    return result
